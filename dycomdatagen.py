...
def dycomrconverterV2(data, dir_mass, dir_mask, end_path_mass, end_path_mask):
    logger.debug(
        f"Per estrarre le immagini e maschere, carico da {dir_mass} e {dir_mask} con cartelle di arrivo {end_path_mass} e {end_path_mask}"
    )
    ...
    try:
        ...
        raise Exception(
            f"Nella directory {dir_masses} delle singole immagini ci sono più file"
        )
        logger.exception(
            f"Nella directory {dir_masses} delle singole immagini ci sono più file"
        )
    ...
    try:
        ...
        logger.exception(
            f"Nella directory {dir_masks} delle maschere ci sono più di due file"
        )
    for f in files:
